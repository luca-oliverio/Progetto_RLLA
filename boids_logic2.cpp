...
void add(std::array<double, 2>& i, const std::array<double, 2>& other)
{
  i[0] += other[0];
  i[1] += other[1];
}
...
Boid cm(const std::vector<Boid>& boids)
{
  if (boids.empty())
    return Boid{}; // evita divisione per zero

  // Somma tutte le posizioni usando accumulate
  Boid total = std::accumulate(boids.begin(), boids.end(), Boid{},
                               [](Boid acc, const Boid& b) {
                                 add(acc.pos, b.pos);
                                 return acc;
                               });
  double n   = static_cast<double>(boids.size());
  return Boid{total.pos[0] / n, total.pos[1] / n}; // velocità impostata a zero
}
...
std::vector<Position>& Movement::get_boids() const
{
  std::vector<Position> boids_positions;
  boids_positions.clear();
    ...
    for (size_t i = 0; i < n_b; ++i) {
      ...
      for (size_t j = 0; j < n_b; ++j) {
        ...
        if (is_neighbor(boids[i].pos, boids[j].pos)) {
          ...
          neighbor_count++;
          add(vel_tot[i], rule1(self.pos, other.pos));
        }
      }
      ...
      if (neighbor_count > 0) {
        ...
        add(vel_tot[i], rule2(boids[i].vel, mean_vel));
        add(vel_tot[i], rule3(self.pos, center_mass));
      }
