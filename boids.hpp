...
class Movement
{
 ...
 public:
  Movement(std::vector<boids> b_, int n_b_, double d_, double d_s_, double s_, double a_,
           double c_)
  {
    b   = b_;
    n_b = n_b_;
  ...
  double velocity(boids_vel boid) { // calcolo il modulo della velocità per ogni boid
    return (sqrt(boid.v_x*boid.v_x+boid.v_y*boid.v_y));
  }
  double diff_pos(boids i, boids j) { // calcolo la differenza tra le posizioni dei boid i e j
  ...
  boids_vel rule1(boids i, boids j) {
    ...
    else return boids_vel(0., 0.);
  };
  boids_vel rule2(boids_vel i, boids_vel j) { //a minore di 1, VERIFICA
    return boids_vel((a/(n_b-1))*(j.v_x-i.v_x),(a/(n_b-1))*(j.v_y-i.v_y));
  };
